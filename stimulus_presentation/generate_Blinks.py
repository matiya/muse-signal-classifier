...
# Setup graphics
mywin = visual.Window([900, 800], monitor='testMonitor', units='deg',
                      fullscr=False)
# Initialize stimuli
...
mov = visual.MovieStim3(mywin,path_to_stimulus, size=(800, 600), volume=1.0, noAudio=True)
start = time()
...
while mov.status != visual.FINISHED:
    ...
    if((round(time() - start,2)%3)<=0.015 and (time() - start)>2):
